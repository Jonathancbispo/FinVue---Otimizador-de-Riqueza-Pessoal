...
import { User, Lock, Save, Loader2, CheckCircle2, AlertCircle, ShieldCheck, Mail, LogOut } from 'lucide-react';
...
const ProfileSettings: React.FC<ProfileSettingsProps> = ({ initialName, email, onLogout }) => {
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20">
      ...
      <div className="pt-8 border-t border-slate-200 dark:border-darkBorder flex justify-center">
        <button 
          onClick={onLogout}
          className="flex items-center space-x-3 px-12 py-4 rounded-3xl text-slate-500 hover:text-rose-600 dark:text-slate-400 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/10 border-2 border-transparent hover:border-rose-100 dark:hover:border-rose-800/30 transition-all font-black uppercase tracking-widest text-[11px]"
        >
          <LogOut size={20} />
          <span>Sair da Conta FinVue</span>
        </button>
      </div>
    </div>
  );
